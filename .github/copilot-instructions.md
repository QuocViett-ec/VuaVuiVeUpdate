...
**auth.middleware.js:**
```js
exports.requireAuth = (req, res, next) => { /* check req.session.userId */ }
exports.requireAdmin = (req, res, next) => { /* check req.session.role === 'admin' */ }
```
...
exports.uploadImage = multer({ storage, limits: { fileSize: 5*1024*1024 } }).single('image');
```
