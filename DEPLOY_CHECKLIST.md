...
- [ ] Customer portal chay duoc: `http://localhost:4200`.
- [ ] Admin portal chay duoc: `http://localhost:4201/auth/login`.
- [ ] VNPay demo chay duoc: `http://localhost:8888`.
- [ ] ML API chay duoc o port 5001.

## 3) Bien moi truong bat buoc

- [ ] `backend/.env` co `MONGO_URI` hop le.
- [ ] `backend/.env` co `SESSION_SECRET` manh.
- [ ] `backend/.env` co `CLIENT_ORIGINS` dung domain/port frontend.
- [ ] `backend/.env` co `RECOMMENDER_API=http://localhost:5001` (hoac URL production tuong ung).
